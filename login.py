    ...
    if elemento is not None and elemento.text:
        ...
        try:
            ...
            try:
                ...
                return valor  # Se não for uma data, retorna o valor original
    else:
        return mensagem_erro

# Função para concatenar valores
def obter_dado_concatenado(aluno, tag1, tag2, mensagem_erro):
    elemento1 = aluno.find(tag1, namespaces)
    elemento2 = aluno.find(tag2, namespaces)
    if elemento1 is not None and elemento2 is not None:
        return f"{elemento1.text}/{elemento2.text}"  # Concatenar os dois valores
    else:
        return mensagem_erro
    ...
    try:
        ...
        if aluno and curso and historico and seguranca and situacao_atual:
            dados_extraidos.append({
                ...
                "Documento de Identidade": f"{obter_dado(aluno, 'ns:RG/ns:Numero', 'Identidade não encontrada')}/{obter_dado(aluno, 'ns:RG/ns:OrgaoExpedidor', 'Órgão expedidor não encontrado')} - {obter_dado(aluno, 'ns:RG/ns:UF', 'UF não encontrada')}",
